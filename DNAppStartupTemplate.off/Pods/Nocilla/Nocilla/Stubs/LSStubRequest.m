...
-(BOOL)matchesBody:(id<LSHTTPRequest>)request {
    ...
    NSString *selfEscBody = [[NSString alloc] initWithData:self.body encoding:NSUTF8StringEncoding];
    selfEscBody = [selfEscBody stringByReplacingOccurrencesOfString:@"\"" withString:@"\\\""];

    NSString *escapedBody = [[NSString alloc] initWithData:[request body] encoding:NSUTF8StringEncoding];
    escapedBody = [escapedBody stringByReplacingOccurrencesOfString:@"\"" withString:@"\\\""];

    if (!selfEscBody || [selfEscBody isEqualToString:escapedBody]) {
        return YES;
    }
    return NO;
}
