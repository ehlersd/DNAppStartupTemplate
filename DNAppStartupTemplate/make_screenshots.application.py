...
def compile_app():
    ...
    build_path = os.path.join(os.getcwd(), options['build_path'])

    # Force the simulator build to use 32-bit, otherwise UIGetScreenImage doesn't exist
    subprocess.call(['xcodebuild', '-workspace', options['workspace_name'], '-scheme', options['scheme_name'], '-configuration', options['build_config'], '-sdk', 'iphonesimulator', 'clean', 'build', 'SYMROOT='+build_path, 'ARCHS=i386', 'ONLY_ACTIVE_ARCH=NO'], stdout=open('/dev/null', 'w'))
    os.chdir(previous_dir)
...
def iossim(app_path, args, device):
    ...
    #for subprocess_arg in subprocess_args:
    #    print 'subprocess_arg = ' + subprocess_arg
...
if __name__ == '__main__':
    ...
    for device in options['devices']:
        ...
        for language in options['languages']:
            ...
            language_path = os.path.join(os.getcwd(), language_path)
